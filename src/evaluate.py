...
def _load_checkpoint(model: Any, ckpt_path: str | None) -> Any:
    ...
    if total_loaded == 0:
        ...
        raise RuntimeError(
            "Couldn't load any parameters from checkpoint into model. "
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Local evaluator for img2gps.")
    p.add_argument("--model", required=True, help="Path to model.py")
    p.add_argument("--preprocess", required=True, help="Path to preprocess.py")
    p.add_argument("--weights", default=None, help="Optional path to model checkpoint")
    p.add_argument("--csv", required=True, help="Path to validation CSV")
...
def main() -> None:
    args = parse_args()
    model_mod = _dynamic_import(args.model, "model_a")
    preproc_mod = _dynamic_import(args.preprocess, "preproc_a")
